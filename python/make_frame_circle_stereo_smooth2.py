...
def make_frame_cir(wave_data_file_name,
    ...
    for frame_index,img_frame_time in enumerate(img_frame_times):
        ...
        for au_img_fit_ind in np.linspace(pre_au_img_fit_ind+1,now_au_img_fit_ind,comp_num):
            print("\r"+str(frame_index)+":"+str(int(au_img_fit_ind)),end="")
            cut_index = np.arange(int(au_img_fit_ind)-int(au_display_sample_num),int(au_img_fit_ind),1)
            cut_index[cut_index < 0] = 0
            cut_index[cut_index >= au_data_time.shape[0]] = ch1_wav.shape[0] - 1
            ...
            img_init_time = int(au_img_fit_ind)/a_sr
            ...
            if ch1_freq == 0:
                ...
                #ch1_demod_phase_post = np.arctan2(ch1_I_filted,ch1_Q_filted)[int(au_display_sample_num/2)]
                ch1_init_phase = (np.mod(img_init_time,1/ch1_freq)*ch1_freq)*2*np.pi
                if ch1_init_phase > np.pi:
                    ch1_init_phase = -2*np.pi + ch1_init_phase
                ch1_demod_phase_temp.append(ch1_demod_phase_post +ch1_init_phase)
                ch1_power_temp.append(np.sqrt(np.max(np.power(ch1_I_filted,2)+np.power(ch1_Q_filted,2))))
            ...
            if ch2_freq == 0:
                ...
                ch2_init_phase = (np.mod(img_init_time,1/ch2_freq)*ch2_freq)*2*np.pi
                if ch2_init_phase > np.pi:
                    ch2_init_phase = -2*np.pi + ch2_init_phase
                ch2_demod_phase_temp.append(ch2_demod_phase_post +ch2_init_phase)
                ch2_power_temp.append(np.sqrt(np.max(np.power(ch2_I_filted,2)+np.power(ch2_Q_filted,2))))
    
            ch1_Freq_peak_temp.append(ch1_freq)
            ch2_Freq_peak_temp.append(ch2_freq)
        
        pre_au_img_fit_ind = now_au_img_fit_ind
        
        ch1_power.append(np.array(ch1_power_temp))
        ch2_power.append(np.array(ch2_power_temp))
        ch1_Freq_peak.append(np.array(ch1_Freq_peak_temp))
        ch2_Freq_peak.append(np.array(ch2_Freq_peak_temp))
