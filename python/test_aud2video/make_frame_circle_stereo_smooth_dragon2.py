...
def make_frame_cir(wave_data_file_name,
    ...
    pre_au_img_fit_ind = 0

    comp_num = 2
    for frame_index,img_frame_time in enumerate(img_frame_times):
        now_au_img_fit_ind = int(img_frame_time* a_sr)
        ch1_power_temp = []
        ch2_power_temp = []
        ch1_data_temp = []
        ch2_data_temp = []
        ...
        ch1_kurt_temp = []
        ch2_kurt_temp = []
        ch1_demod_phase_temp =[]
        ch2_demod_phase_temp =[]

        for au_img_fit_ind in np.linspace(pre_au_img_fit_ind+1,now_au_img_fit_ind,comp_num):
            print("\rframe:"+str(frame_index)+":"+str(au_img_fit_ind),end="")
            au_img_fit_ind = int(au_img_fit_ind)
            ...
            ch1_power_temp.append(0.1)   
            ch2_power_temp.append(0.1)
            
            multi_power = 3
            ch1_m_data = np.abs(ch1_hilbert_signal[cut_index])*multi_power
            ch2_m_data = np.abs(ch2_hilbert_signal[cut_index])*multi_power
            
            FFT_window = np.hamming(au_display_sample_num)
            ch1_FFT_wav = np.concatenate([ch1_wav[cut_index]*FFT_window,np.zeros_like(ch1_wav[cut_index]*3)],axis=0)
            ch2_FFT_wav = np.concatenate([ch2_wav[cut_index]*FFT_window,np.zeros_like(ch2_wav[cut_index]*3)],axis=0)
            ...
            if ch1_freq == 0:
                ch1_demod_phase_temp.append(0)
                ch1_data_temp.append(0)
            else:
                ...
                ch1_demod_phase_temp.append(np.mod(ch1_demod_phase_post +ch1_init_phase,2*np.pi)-np.pi)
                ch1_data_temp.append(np.sqrt(np.max(np.power(ch1_I_filted,2)+np.power(ch1_Q_filted,2))))
            ...
            if ch2_freq == 0:
                ch2_demod_phase_temp.append(0)
                ch2_data_temp.append(0)
            else:
                ...
                ch2_demod_phase_temp.append(np.mod(ch2_demod_phase_post +ch2_init_phase,2*np.pi)-np.pi)
                ch2_data_temp.append(np.sqrt(np.max(np.power(ch2_I_filted,2)+np.power(ch2_Q_filted,2))))
            
            ch1_var_temp.append(np.var(ch1_m_data))
            ch2_var_temp.append(np.var(ch2_m_data))

            if np.sum(ch1_m_data-np.mean(ch1_m_data)) < pow(10,-8):
                ch1_skew_temp.append(np.nan)
                ch1_kurt_temp.append(np.nan)
            else:
                ch1_skew_temp.append(stats.skew(ch1_m_data))
                ch1_kurt_temp.append(stats.kurtosis(ch1_m_data))
            if np.sum(ch2_m_data-np.mean(ch2_m_data)) < pow(10,-8):
                ch2_skew_temp.append(np.nan)
                ch2_kurt_temp.append(np.nan)
            else:
                ch2_skew_temp.append(stats.skew(ch2_m_data))
                ch2_kurt_temp.append(stats.kurtosis(ch2_m_data))        

        pre_au_img_fit_ind = now_au_img_fit_ind
        
        ch1_power.append(np.array(ch1_power_temp))
        ch2_power.append(np.array(ch2_power_temp))
        ch1_data.append(np.array(ch1_data_temp))
        ch2_data.append(np.array(ch2_data_temp))
        ch1_var.append(np.array(ch1_var_temp))
        ch2_var.append(np.array(ch2_var_temp))
        ch1_skew.append(np.array(ch1_skew_temp))
        ch2_skew.append(np.array(ch2_skew_temp))
        ch1_kurt.append(np.array(ch1_kurt_temp))
        ch2_kurt.append(np.array(ch2_kurt_temp))
        ch1_demod_phase.append(np.array(ch1_demod_phase_temp))
        ch2_demod_phase.append(np.array(ch2_demod_phase_temp))
    ...
    for index in range(ch1_size_params.shape[0]):
        ...
        ch1_r_posi_param = ch1_r_posi_params[index]
        ch2_r_posi_param = ch2_r_posi_params[index]
        

        ch1_the_posi_param = ch1_the_posi_params[index]
        ch2_the_posi_param = ch2_the_posi_params[index]
        f.write(str(ch1_the_posi_param)+"\n")
